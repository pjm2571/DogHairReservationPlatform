...
from .models import MarketPost, Recommended_MarketPost
...
def market_post_list(request):
    ...
    if query:
        ...
        searchPosts = MarketPost.objects.all().order_by('-created_at')
...
def market_post_detail(request, pk):
    ...
    if Recommended_MarketPost.objects.filter(user=user, post=post).exists():
...
class market_post_like(APIView):
    def post(self, request):
        user_id = request.data.get('user_id', None)
        post_id = request.data.get('post_id', None)

        user = User.objects.filter(id=user_id).first()
        post = MarketPost.objects.filter(id=post_id).first()
        ...
        Recommended_MarketPost.objects.create(
            user=user,
            post=post
        )
