...
from .models import BoastPost, Recommended_BoastPost
...
def boast_post_list(request):
    ...
    if query:
        searchPosts = BoastPost.objects.order_by('-created_at').filter(  # -created_at은 게시물 역순으로 정렬
            Q(title__icontains=query) |
            Q(content__icontains=query)
        )
    else:
        searchPosts = BoastPost.objects.all().order_by('-created_at')
...
def boast_post_detail(request, pk):
    ...
    if Recommended_BoastPost.objects.filter(user=user, post=post).exists():
        context['is_Recommended'] = True

    return render(request, 'boastboard/boast_post_detail.html', context)
...
class boast_post_like(APIView):
    def post(self, request):
        user_id = request.data.get('user_id', None)
        post_id = request.data.get('post_id', None)

        user = User.objects.filter(id=user_id).first()
        post = BoastPost.objects.filter(id=post_id).first()
        ...
        Recommended_BoastPost.objects.create(
            user=user,
            post=post
        )
